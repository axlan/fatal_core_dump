...
int main()
{
    SDNHandler callback_handlers[] = {
        {.type = SDN_MSG_TYPE_HEARTBEAT,
         .callback = HandleHeartBeat},
        {.type = SDN_MSG_TYPE_SENSOR_PRESSURE,
         .callback = HandleSensorPressure},
        {.type = SDN_MSG_TYPE_CLEAR_FAULTS,
         .callback = HandleClearFaults},
#if APP_DEBUG_BUILD
        {.type = SDN_MSG_TYPE_DEBUG_WRITE_CONFIG_INT,
         .callback = HandleDebugWriteConfigInt},
#endif
        {.type = SDN_MSG_TYPE_SET_AIRLOCK_OPEN,
    ...
    const size_t num_callback_handlers = sizeof(callback_handlers) / sizeof(SDNHandler);
    ...
    *state.airlock_pressures[1] = NAN;
    ...
    while (true)
    {
        if (ProcessMessageData(callback_handlers, num_callback_handlers, message_buffer, state.config.message_buffer_size) < 0)
        {
            return 5;
        }
